...
extern osThreadId_t h_receiveUpdateHandle;
    ...
    s_uart_buffer[s_uart_index] = byte;
		...
		if(s_uart_index == 4){
			...
			if(s_len_payload > 0){
				...
				if ((s_last_byte == ETX_OTA_SALTO_LINEA) && (byte == ETX_OTA_FIN_LINEA)) {  // fin de paquete
						s_packet_ready = 1;
		...
		if(s_packet_ready){
			memcpy_s((void*)g_rx_buffer, ETX_OTA_PACKET_MAX_SIZE, s_uart_buffer, s_uart_index);

			s_uart_index = 0;
			s_packet_ready = 0;
			osThreadFlagsSet(h_receiveUpdateHandle, FLAG_RECEIVE_UPDATE);
		}
